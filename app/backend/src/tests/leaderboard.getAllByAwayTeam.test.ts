...
import {
  ...
  leaderboardGetAllByAwayTeamResponse,
...
describe('Testes da rota "GET /leaderboard/away"', () => {
  ...
  describe('Será validado que é possível filtrar as classificações dos times visitantes', () => {
    ...
    it('Deve responder com uma lista contendo os placares das partidas finalizadas filtrada pelos times visitantes', () => {
      expect(chaiHttpResponse.body).to.be.eqls(leaderboardGetAllByAwayTeamResponse);
    });
  });
});